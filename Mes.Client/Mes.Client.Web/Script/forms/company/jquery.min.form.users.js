        ...
        events: {
            ...
            loadData: function () {
                usersForm.controls.dgdetail.datagrid({
                    ...
                    {
                        field: 'Email', title: '电子邮件', width: 150, sortable: false, halign: 'center', align: 'left', formatter: function (value, rowData, rowIndex) {
                            if (value != "") {
                    ...
                    { field: 'IDNumber', title: '证件号码', width: 150, sortable: false, align: 'center' },
                    {
                        field: 'IsLocked', title: '锁定', width: 100, sortable: false, align: 'center', formatter: function (value, rowData, rowIndex) {
                            if (value == "True") {
                    ...
                    {
                        field: 'IsDisabled', title: '禁用', width: 100, sortable: false, align: 'center', formatter: function (value, rowData, rowIndex) {
                            if (value == "True") {
                                return "<span style='color:red;'>√</span>";
                            }
                            return "";
            ...
            UpdateUser: function () {
                ...
                if (selectedRows.length > 0) {
                    $.ajax({
                        url: '/ashx/usershandle.ashx?Method=GetUser&UserID=' + selectedRows[0]['UserID'] + '&OldPasswordID=' + selectedRows[0]["Password"],
                        success: function (data) {
                            usersForm.controls.edit_form.form('load', data);
                            usersForm.controls.edit_form.find('#IsDisabled').attr('checked', data["IsDisabled"] == 'True' ? true : false);
                            usersForm.controls.edit_form.find('#IsDisabled').val(data["IsDisabled"]);
                            usersForm.controls.edit_form.find('#IsLocked').attr('checked', data["IsLocked"] == 'True' ? true : false);
            ...
            PWDConfirm: function () {
                var selectedRows = usersForm.controls.dgdetail.datagrid('getSelections');
                if (selectedRows.length > 0) {
                    usersForm.controls.win_changepwd.window('open');
                    if (usersForm.controls.win_changepwd_form.form('validate')) {
                        $.ajax({
                            url: '/ashx/usershandle.ashx?Method=ChangPsw&UserID=' + selectedRows[0]['UserID'] + '&OldPassword=' + usersForm.controls.win_changepwd_form.find("#OldPassword").val() + '&NewPassword=' + usersForm.controls.win_changepwd_form.find("#NewPassword").val(),
                            success: function (data) {
                                if (data) {
                                    if (data.isOk == 1) {
                                        showInfo('恭喜，密码修改成功！');
                                        usersForm.controls.win_changepwd_form.form('clear');
