(function ($) {
    ...
    var cabinetwareForm = {
        init: function () {
            cabinetwareForm.initControls();
            cabinetwareForm.events.loadData();
            cabinetwareForm.controls.searchData.on('click', cabinetwareForm.events.loadData);//加载数据           
            cabinetwareForm.events.loadCombobox();
        },
        initControls: function () {
            cabinetwareForm.controls = {
        ...
        events: {
            ...
            loadData: function () {
                cabinetwareForm.controls.dgdetail.datagrid({
                    idField: 'MaterialID',
                    ...
                    onBeforeLoad: function (param) {
                        cabinetwareForm.controls.search_form.find('select').each(function (index) {
                            param[this.name] = $(this).val();
                        });
                        cabinetwareForm.controls.search_form.find('input').each(function (index) {
                            if (this.name != "")
                                param[this.name] = $(this).val();
                        });
                    }
