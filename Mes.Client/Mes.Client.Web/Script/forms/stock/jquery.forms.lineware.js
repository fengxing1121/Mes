(function ($) {
    ...
    var linewareForm = {
        init: function () {
            linewareForm.initControls();
            linewareForm.events.loadData();
            linewareForm.controls.searchData.on('click', linewareForm.events.loadData);//加载数据           
            linewareForm.events.loadCombobox();
        },
        initControls: function () {
            linewareForm.controls = {
        ...
        events: {
            ...
            loadData: function () {
                linewareForm.controls.dgdetail.datagrid({
                    idField: 'MaterialID',
                    ...
                    onBeforeLoad: function (param) {
                        linewareForm.controls.search_form.find('select').each(function (index) {
                            param[this.name] = $(this).val();
                        });
                        linewareForm.controls.search_form.find('input').each(function (index) {
                            if (this.name != "")
                                param[this.name] = $(this).val();
                        });
                    }
