(function ($) {
    ...
    var hardwareForm = {
        init: function () {
            hardwareForm.initControls();
            hardwareForm.events.loadData();
            hardwareForm.controls.searchData.on('click', hardwareForm.events.loadData);//加载数据           
            hardwareForm.events.loadCombobox();
        },
        initControls: function () {
            hardwareForm.controls = {
        ...
        events: {
            ...
            loadData: function () {
                hardwareForm.controls.dgdetail.datagrid({
                    idField: 'MaterialID',
                    ...
                    onBeforeLoad: function (param) {
                        hardwareForm.controls.search_form.find('select').each(function (index) {
                            param[this.name] = $(this).val();
                        });
                        hardwareForm.controls.search_form.find('input').each(function (index) {
                            if (this.name != "")
                                param[this.name] = $(this).val();
                        });
                    }
