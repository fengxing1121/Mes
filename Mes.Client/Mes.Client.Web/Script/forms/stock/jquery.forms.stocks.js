(function ($) {
    ...
    var stocksForm = {
        init: function () {
            stocksForm.initControls();
            stocksForm.events.loadData();
            stocksForm.controls.searchData.on('click', stocksForm.events.loadData);//加载数据           
            stocksForm.events.loadCombobox();
        },
        ...
        events: {
            ...
            loadData: function () {
                stocksForm.controls.dgdetail.datagrid({
                    idField: 'MaterialID',
                    ...
                    onBeforeLoad: function (param) {
                        stocksForm.controls.search_form.find('select').each(function (index) {
                            param[this.name] = $(this).val();
                        });
                        stocksForm.controls.search_form.find('input').each(function (index) {
                            if (this.name != "")
                                param[this.name] = $(this).val();
                        });
                    }
