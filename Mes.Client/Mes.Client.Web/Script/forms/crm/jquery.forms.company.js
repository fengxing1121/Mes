        ...
        events: {
            loadData: function () {
                partnerForm.controls.dgdetail.datagrid({
                    url: '/ashx/companyhandler.ashx?Method=SearchCompanys&' + jsNC(),
                    collapsible: false,
                    fitColumns: true,
                    pagination: true,
                    ...
                    columns: [[
                                ...
                                { field: 'CompanyName', title: '工厂名称', width: 150, align: 'center' },
                                ...
                                { field: 'Address', title: '联系地址', width: 120, sortable: false, align: 'center' }, 
                    ]],
            ...
            savepartner: function () {
                if (partnerForm.controls.edit_form.form('validate')) {
                    switch (addOrUpdate) {
                        case "add":
                            $.ajax({
                                url: '/ashx/companyhandler.ashx?Method=SaveCompany',
                        ...
                        case "update":
                            $.ajax({
                                url: '/ashx/companyhandler.ashx?Method=ModifyCompany',
                                data: partnerForm.controls.edit_form.serialize(),
                                success: function (returnData) {
                                    if (returnData) {
                                        if (returnData.isOk == 1) {
            ...
            updatesupplier: function () {
                ...
                if (selectedRows.length > 0) {
                    $.ajax({
                        url: '/ashx/companyhandler.ashx?Method=GetCompany&CompanyID=' + selectedRows[0]['CompanyID'],
                        success: function (data) {
                            ...
                            partnerForm.controls.edit_form.find("#Status").combobox('setValue', row.Status); //加载状态
