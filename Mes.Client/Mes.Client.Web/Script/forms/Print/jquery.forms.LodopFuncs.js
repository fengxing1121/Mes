...
function getLodop(oOBJECT, oEMBED) {
    ...
    try {
        ...
        if (needCLodop()) {
            ...
            if (!LODOP) {
                if (isIE) alert(strCLodopInstall); else
                    alert(strCLodopInstall);
                return;
            } else {

                if (CLODOP.CVERSION < "3.0.2.9") {
                    if (isIE) alert(strCLodopUpdate); else
            ...
            if ((LODOP == null) || (typeof (LODOP.VERSION) == "undefined")) {
                ...
                if (navigator.userAgent.indexOf('Firefox') >= 0)
                    alert(strHtmFireFox);
                if (is64IE) alert(strHtm64_Install); else
                    if (isIE) alert(strHtmInstall); else
                        alert(strHtmInstall);
                return LODOP;
            };
        };
        if (LODOP.VERSION < "6.2.2.1") {
            if (!needCLodop()) {
                if (is64IE) alert(strHtm64_Update); else
