    ...
    events: {
        ...
        loadData: function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.StepNo != GetOrderStepNo(partneraddorder)) {
            ...
            $.messager.confirm('系统提示', '您确定要修改该订单吗?', function (flag) {
                if (flag) {
                    editorderForm.controls.edit_form.form('submit', {
                        url: '/ashx/PartnerOrderHandler.ashx?Method=SavePartnerOrder&edit=true&' + jsNC(),
        ...
        cancelorder: function () {
            ...
            $.messager.confirm('系统提示', '您确定要取消该订单吗?', function (flag) {
                if (flag) {
                    $.ajax({
                        url: '/ashx/PartnerOrderHandler.ashx?Method=CancelOrder',
                        data: { OrderID: orderid },
                        success: function (returnData) {
                            if (returnData) {
                                if (returnData.isOk == 0) {
...
function cancelrow(id,index) {
    ...
    $.ajax({
        url: '/ashx/PartnerOrderHandler.ashx?Method=DeleteByProductID&' + jsNC(),
        data: { ProductID: id },
        datatype: "json",
    });
}
