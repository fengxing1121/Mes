    ...
    events: {
        loadData: function () {
            ordersForm.controls.dgdetail.datagrid({
                sortName: 'Created',
                idField: 'OrderID',
                url: '/ashx/PartnerOrderHandler.ashx?Method=SearchPartnerOrder&Review=true&' + jsNC(),
                collapsible: false,
                fitColumns: true,
                pagination: true,
                ...
                columns: [[
                    {
                        field: 'OrderNo', title: '订单编号', width: 120, align: 'center', formatter: function (value, rowData, rowIndex) {
                            var strReturn = '<a href="javascript:void(0)" class="l-btn-text" title="审核订单" onclick="ordersForm.events.showdetail(\'' + rowData['OrderID'] + '\',\'' + rowData['OrderNo'] + '\',\'' + rowData['StepNo'] + '\');"><span style="color:#0094ff;">' + rowData['OrderNo'] + '</span></a>';
                            return strReturn;
                        }
                    },
        ...
        showdetail: function (id, no, stepno) {
            top.addTab("审核【" + no + "】", "/View/PartnerOrder/ordersdetail.aspx?orderid=" + id + "&stepno=" + stepno + "&source=confirm", 'table');
