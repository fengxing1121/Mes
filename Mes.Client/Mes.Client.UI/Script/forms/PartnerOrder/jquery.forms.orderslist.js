    ...
    events: {
        loadData: function () {
            ordersForm.controls.dgdetail.datagrid({
                ...
                columns: [[
                    {
                        field: 'OrderNo', title: '订单编号', width: 125, align: 'center', formatter: function (value, rowData, rowIndex) {
                    ...
                    { field: 'SalesMan', title: '业务人员', width: 100, sortable: false, halign: 'center', align: 'center' },
                    {
                        field: 'Status', title: '订单状态', width: 110, align: 'center', formatter: function (value, rowData, rowIndex) {
                            var strReturn = '<a href="javascript:void(0)" class="l-btn-text" title="审核明细" onClick="loadstep(\'' + rowData['OrderID'] + '\');"><span style="color:#0094ff;">' + GetOrderStatusName(value)+ '</span></a>';
                            return strReturn;
                        }
                    }
                ]],
        ...
        showdetail: function (id, no) {
            top.addTab("查看【" + no + "】", "/View/PartnerOrder/ordersdetail.aspx?orderid=" + id);
        },
