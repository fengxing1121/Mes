    ...
    events: {
        loadData: function () {
            ordersForm.controls.dgdetail.datagrid({
                ...
                toolbar: [
                    '-',
                  { text: '完成设计', iconCls: 'icon-ok', handler: ordersForm.events.applyorder }
                ],
        ...
        applyorder: function () {
            ...
            $.messager.confirm('系统提示', '您确定所选订单已完成设计?', function (flag) {
                if (flag) {
                    $.ajax({
                        url: '/ashx/ordershandler.ashx?Method=OrdersDesign',
                        ...
                        success: function (returnData) {
                            if (returnData) {
                                if (returnData.isOk == 0) {
                                    ...
                                    $('#steps_window').window('close');
                                    ordersForm.controls.dgdetail.datagrid('reload');
                                }
                            }
                        }
                    });
