...
namespace Mes.Client.UI.Ashx
{
    ...
    public class PrivilegeHandler : BaseHttpHandler
    {
        ...
        /// 门店端限制只显示CRM和销售预订单
        /// </summary>
        private  List<Guid> CategoryIDList = new List<Guid>() { new Guid("93B305E5-FF00-5A4F-D2E0-4C2F86FA861A"), new Guid("9DA97EEC-5DDA-343E-1FB0-92027B3B8DC9") };
        ...
        public void Tree()
        {
            try
            {
                using (ProxyBE p = new ProxyBE())
                {
                    ...
                    {
                        StringBuilder jsonBuilder = GetTreeBuilder(p, PrivilegeItems,false);
        ...
        public StringBuilder GetTreeBuilder(ProxyBE p, List<PrivilegeItem> PrivilegeItems,bool IsPartner=true)
        {
            ...
            for (int n = 0; n < privilegeCategorys.Count; n++)
            {
                ...
                if (!CategoryIDList.Contains(item.CategoryID)&& IsPartner) continue;
            ...
            try
            {
                using (ProxyBE p = new ProxyBE())
                {
                    ...
                    if (CurrentUser.UserType == (int)UserType.D)
                    {
                        ...
                        args.CategoryIDs = CategoryIDList;
                    }
