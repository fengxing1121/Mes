    ...
    public class CategoryHandler : BaseHttpHandler
    {
        ...
        public void GetOrderType()
        {
            try
            {
                using (ProxyBE p = new ProxyBE())
                {
                    Category category = p.Client.GetCategoryByParentID_CategoryCode(SenderUser, CategoryRootID, "OrderType");
                    if (category != null)
                    {
                        List<Category> lists = p.Client.GetCategoriesByParentID(SenderUser, category.CategoryID);

                        if (string.IsNullOrEmpty(Request["whole"]))
                        {
        ...
        public void GetOrderStatus()
        {
            try
            {
                using (ProxyBE p = new ProxyBE())
                {
                    Category category = p.Client.GetCategoryByParentID_CategoryCode(SenderUser, CategoryRootID, "OrderStatus");
                    if (category != null)
                    {
                        List<Category> lists = p.Client.GetCategoriesByParentID(SenderUser, category.CategoryID);
                        if (string.IsNullOrEmpty(Request["whole"]))
